...
// Icons
import { Plus, Check, Upload, Loader2, X, Trash2, MapPin } from "lucide-react";
...
const AddItem = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    specification: "",
    manufacturer_id: "",
    manufacturer_name: "", // New field for manual manufacturer input
    category_id: "",
    unit_id: null,
    reorder_level: "0", // Global reorder point
    is_serialized: false,
    is_returnable: false,
    unit_cost: "",
    currency: "PKR",
  ...
  const locationOptions = locations
    .map((loc) => ({
      value: loc.id.toString(), // Convert to string explicitly
      label: loc.name,
      is_active: true,
    }))
    .filter((loc) => loc.is_active); // Only show active locations
  ...
  const mapOptions = (arr) =>
    arr?.map((u) => ({ value: u.name, label: u.name })) || [];

  const weightOptions = mapOptions(weightUnits.weight_units);
  const lengthOptions = mapOptions(weightUnits.length_units);
  const widthOptions = mapOptions(weightUnits.width_units);
  const heightOptions = mapOptions(weightUnits.height_units);

  const currencyOptions = [{ value: "PKR", label: "PKR (RS.)" }];
  ...
  useEffect(() => {
    const fetchDropdowns = async () => {
      try {
        ...
        setAccounts(accsRes || []);
        setManufacturers(mansRes || []);
        setVendors(vendorsRes || []);
        setUnits(unitsRes || []);
        setWeightUnits(weightUnitRes || []);
        console.log("all units data", weightUnitRes);
        console.log("locations", locsRes);
      } catch (err) {
  ...
  const handleManufacturerChange = (selectedOption, actionMeta) => {
    if (actionMeta.action === "create-option") {
      // User is creating a new manufacturer
      setFormData((prev) => ({
        ...prev,
        manufacturer_id: "", // Clear the ID since it's a new manufacturer
        manufacturer_name: selectedOption.value, // Store the manual input
      }));
      ...
      setFormData((prev) => ({
        ...
        manufacturer_name: "", // Clear manual name when selecting existing
      }));
  ...
  const handleLocationSelectChange = (index, selectedOption) => {
    setItemLocations((prev) =>
      prev.map((loc, i) =>
        i === index
          ? { ...loc, location_id: selectedOption ? selectedOption.value : "" }
          : loc
      )
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Debug: Check what's in itemLocations
      console.log("Item Locations before submission:", itemLocations);
      console.log("Item Locations JSON string:", JSON.stringify(itemLocations));
      ...
      // Debug: Check locations before appending
      console.log("Locations to be sent:", itemLocations);
      console.log("Is locations array?", Array.isArray(itemLocations));
      console.log("Locations length:", itemLocations.length);

      // Append locations data - FIXED VERSION
      if (itemLocations.length > 0) {
        // Ensure all location IDs are strings and validate data
        const validLocations = itemLocations
          .filter(
            (loc) => loc.location_id && loc.location_id.toString().trim() !== ""
          )
          .map((loc) => ({
            location_id: loc.location_id.toString(), // Ensure string
            aisle: loc.aisle || "",
            row: loc.row || "",
            bin: loc.bin || "",
            quantity: parseInt(loc.quantity) || 0,
            reorder_level: parseInt(loc.reorder_level) || 0,
            is_active: Boolean(loc.is_active),
          }));

        console.log("Valid locations after processing:", validLocations);

        if (validLocations.length > 0) {
          form.append("locations", JSON.stringify(validLocations));
        } else {
          // If no valid locations, send empty array
          form.append("locations", JSON.stringify([]));
        }
      } else {
        // Send empty array if no locations
        form.append("locations", JSON.stringify([]));
      }
            ...
            <form onSubmit={handleSubmit} className="p-6">
              ...
              <Card className="mb-6 border-0 shadow-none">
                <CardHeader className="pb-4">
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Add multiple locations with specific quantities and reorder
                    points
                  </p>
                </CardHeader>
                <CardContent>
                        ...
                        >
                          <div className="flex justify-between items-center mb-4">
                            ...
                            <Button
                              ...
                              variant="outline"
                              size="sm"
                              className="text-red-600 bg-red-500 hover:bg-red-100 text-white"
                            >
                        ...
                        <div>
                          <p className="text-sm font-medium text-blue-900">
                            ...
                            <span className="font-bold">
                              {itemLocations.reduce(
                                (sum, loc) =>
                                  sum + (parseInt(loc.quantity) || 0),
                                0
                              )}
                            </span>
                          </p>
                          <p className="text-xs text-blue-700 mt-1">
                            {
                              itemLocations.filter((loc) => loc.location_id)
                                .length
                            }{" "}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
                    ...
                    <div className="space-y-2 md:col-span-1">
                      ...
                      <Select
                        ...
                        value={
                          weightOptions.find(
                            (o) => o.value === formData.weight_unit
                          ) || null
                        }
                        onChange={(e) =>
                          setFormData({ ...formData, weight_unit: e.value })
                        }
                        placeholder="Select Unit"
                      />
                    </div>
                    ...
                    <div className="space-y-2 md:col-span-1">
                      ...
                      <Select
                        ...
                        value={
                          lengthOptions.find(
                            (o) => o.value === formData.length_unit
                          ) || null
                        }
                        onChange={(e) =>
                          setFormData({ ...formData, length_unit: e.value })
                        }
                        placeholder="Select Unit"
                      />
                    </div>
                    ...
                    <div className="space-y-2 md:col-span-1">
                      ...
                      <Select
                        ...
                        value={
                          widthOptions.find(
                            (o) => o.value === formData.width_unit
                          ) || null
                        }
                        onChange={(e) =>
                          setFormData({ ...formData, width_unit: e.value })
                        }
                        placeholder="Select Unit"
                      />
                    </div>
                    ...
                    <div className="space-y-2 md:col-span-1">
                      ...
                      <Select
                        ...
                        value={
                          heightOptions.find(
                            (o) => o.value === formData.height_unit
                          ) || null
                        }
                        onChange={(e) =>
                          setFormData({ ...formData, height_unit: e.value })
                        }
                        placeholder="Select Unit"
                      />
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-col items-center space-y-4">
                      <div className="flex flex-col items-center space-y-4">
                        <div className="relative">
                          <div
                            className="w-32 h-32 rounded-lg overflow-hidden cursor-pointer border-2 border-gray-200"
                            onClick={() =>
              ...
              <div className="flex gap-3 justify-end pt-6 border-t">
                ...
                <Button
                  type="submit"
                  disabled={loading}
                  className="h-10 px-8 text-sm bg-primary-color hover:bg-primary-color-hover text-white"
                >
                  {loading ? (
                    <>
