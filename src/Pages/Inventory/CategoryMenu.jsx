...
const CategoryMenu = () => {
  ...
  const handleCategorySelect = (category) => {
    ...
    // You can use the selected category ID for your API calls or state management
  ...
  // Get current active categories for each panel
  const getParentCategories = () => {
    let items = categories;
    if (searchTerm.trim()) {
      // If main search is active, use filtered categories
      items = filterCategories(categories, searchTerm);
    }
    // Apply parent panel search
  ...
  const handleSearchResultClick = (category) => {
    // Find the parent chain to set active states
  ...
  if (loading) {
    return (
      <div className="max-w-xs">
        <div className="w-full p-3 bg-gray-200 rounded-md animate-pulse">
  ...
  return (
    <div className="max-w-xs relative">
      ...
      {/* Main Dropdown Menu */}
      {isOpen && (
        <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded-md shadow-lg z-50 min-w-96">
          {/* Main Search Bar */}
