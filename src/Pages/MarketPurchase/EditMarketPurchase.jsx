...
const EditMarketPurchase = () => {
  const navigate = useNavigate();
  const { id } = useParams(); // Get MPN ID from URL
  ...
  const fetchMarketPurchaseData = async () => {
    setLoadingData(true);
    try {
      const response = await marketPurchaseApi.getById(id);
      console.log("Market Purchase Data:", response);
      ...
      if (response.success && response.data) {
        ...
        // Set existing invoice for display
        if (purchase.invoice_picture) {
          setExistingInvoice(
            purchase.invoice_picture_url || purchase.invoice_picture
          );
        }

        // Set items - handle both direct items and nested items
        if (purchase.items && purchase.items.length > 0) {
          const itemsWithDestinations = purchase.items.map((item) => ({
            id: item.id, // Keep the item ID for updates
            ...
            approved_quantity: parseFloat(item.approved_quantity) || 0,
            quantity_remaining: parseFloat(item.quantity_remaining) || 0,
            quantity_purchased: item.quantity_purchased
              ? parseFloat(item.quantity_purchased).toString()
              : "",
            unit_price: item.unit_price
              ? parseFloat(item.unit_price).toString()
              : "",
            purchase_status: item.purchase_status || "Scheduled",
            purchase_remarks: item.purchase_remarks || "",
            picture: item.picture || "",
            destination_id: item.destination_id || "",
            source_location_id: item.source_location_id || purchase.location_id,
            source_location_name:
              item.source_location_name || "Unknown Location",
          }));
          setSelectedItems(itemsWithDestinations);
        }
      } else {
        throw new Error("Failed to load market purchase data");
      }
    } catch (error) {
      console.error("Error fetching market purchase:", error);
      Toast.fire({
        icon: "error",
        title: "Failed to load market purchase data",
      });
  ...
  const addItem = (option) => {
    ...
    console.log("Selected Demand Data:", demand);
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      // Clear existing invoice preview when new file is selected
      setExistingInvoice("");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Updating market purchase:", submitData);

      // This will now use the correct PUT endpoint
      const response = await marketPurchaseApi.update(id, submitData);

      if (response.success) {
        Toast.fire({
          icon: "success",
          title: "Market Purchase updated successfully",
        });
        navigate("/market-purchases");
      }
    } catch (error) {
      ...
      Toast.fire({
        icon: "error",
        title: error.response?.data?.message || "Failed to update purchase",
      });
    } finally {
      setLoading(false);
    }
          ...
          <main className="flex-1 p-6 overflow-y-auto">
            <div className="flex justify-center items-center h-64">
              <div className="text-center">
                ...
                <p className="text-gray-600">Loading market purchase data...</p>
              </div>
            </div>
          </main>
        </div>
      ...
      <div className="flex flex-col flex-1">
        ...
        <main className="flex-1 p-6 overflow-y-auto">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">
                  Edit Market Purchase {id && `(#${id})`}
                </h1>
                <p className="text-gray-600 mt-1">
                  Update market purchase details and items
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>Add/Edit Items</CardTitle>
              </CardHeader>
              <CardContent>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className="h-10"
                    />
                    {existingInvoice && !mpnData.invoice_picture && (
                      <div className="mt-2">
                    ...
                    {mpnData.invoice_picture && (
                      <p className="text-xs text-green-600 mt-1">
                        New file selected: {mpnData.invoice_picture.name}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      className="w-full p-2 border border-gray-300 rounded-md min-h-[80px] resize-vertical text-sm"
                    />
                  </div>
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        ...
                        Update Purchase ({selectedItems.length})
                      </>
                    )}
                  </Button>
                </div>
...
export default EditMarketPurchase;
