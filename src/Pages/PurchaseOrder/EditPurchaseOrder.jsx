...
const EditPurchaseOrder = () => {
  ...
  const handleAddDemandItems = (lineItems) => {
    ...
    console.log("Current items in dropdown:", items);
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const itemOptions = itemsData.map((item) => ({
        ...
        unit_name: item.unit?.name || "", // Get unit name from nested unit object
      }));
  ...
  const fetchPurchaseOrder = async () => {
    setIsLoading((prev) => ({ ...prev, purchaseOrder: true }));
    try {
      const result = await purchaseOrderAPI.getById(id);
      if (result.success) {
        const poData = result.data;

        // Check for different possible field names for line items
        const lineItemsData =
          poData.line_items ||
          poData.line_items_data ||
          poData.items ||
        ...
        const transformedData = {
          ...
          vendor_id:
            poData.vendor_id?.toString() || poData.vendor?.id?.toString() || "",
          vendor_address: poData.vendor_address || "",
          location_id:
            poData.location_id?.toString() ||
            poData.location?.id?.toString() ||
            "",
          delivery_date: poData.delivery_date
            ? poData.delivery_date.split("T")[0]
            : "",
          contact_person_name: poData.contact_person_name || "",
          contact_person_mobile: poData.contact_person_mobile || "",
          ref_quotation_no: poData.ref_quotation_no || "",
          intco_term: poData.intco_term || "",
          label: poData.label || "",
          specification: poData.specification || "",
          line_items:
            lineItemsData.length > 0
              ? lineItemsData.map((item, index) => ({
                  ...
                  uom: item.uom || item.unit_of_measure || item.unit || "",
          ...
          terms_conditions:
            poData.terms_conditions || formData.terms_conditions,
          subtotal: parseFloat(poData.subtotal) || 0,
          gst_rate: parseFloat(poData.gst_rate) || 18,
          gst_amount: parseFloat(poData.gst_amount) || 0,
          total_after_tax: parseFloat(poData.total_after_tax) || 0,
          wht_rate: parseFloat(poData.wht_rate) || 5,
          wht_amount: parseFloat(poData.wht_amount) || 0,
          total_payable: parseFloat(poData.total_payable) || 0,
          in_words: poData.in_words || "Zero Rupees Only",
          load_weight: poData.load_weight || "",
          empty_weight: poData.empty_weight || "",
        ...
        // Set vehicle weight checkboxes based on existing data
        setShowLoaded(!!poData.load_weight);
        setShowEmpty(!!poData.empty_weight);

        // Calculate totals after setting form data
        setTimeout(() => {
          calculateTotals(transformedData.line_items);
        }, 100);

        // Set existing attachments if any
        if (poData.attachments) {
          setExistingAttachments(poData.attachments);
        }
      } else {
      ...
      setIsLoading((prev) => ({ ...prev, purchaseOrder: false }));
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Ensure numbers are sent as strings but properly formatted
      ...
      const result = await purchaseOrderAPI.update(id, submitData);
                    ...
                    <table className="min-w-full text-sm md:text-base border-collapse">
                      <thead className="bg-gray-50 border-b border-gray-200">
                        <tr>
                          ...
                          <th className="w-[13%] px-2 py-2 text-left font-semibold text-gray-700 text-xs">
                            Amount
                          </th>
                      ...
                      <tbody>
                        {formData.line_items.map((item, index) => (
                          ...
                          >
                            ...
                            {/* Amount */}
                            <td className="px-2 py-2 text-right font-semibold text-gray-800 text-sm">
