...
const EditWht = () => {
  ...
  const fetchWht = async () => {
    setFetching(true);
    try {
      const response = await whtAPI.getById(id);
      if (response.data) {
        const wht = response.data;
        setFormData({
          name: wht.name || "",
          value: wht.value?.toString() || "",
        });
      } else {
        toast.error("WHT not found");
        navigate("/wht");
      }
    } catch (error) {
      console.error("Fetch WHT error:", error);
      toast.error(error.response?.data?.message || "Failed to load WHT");
      navigate("/wht");
    } finally {
      setFetching(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await whtAPI.update(id, {
        ...formData,
        value: parseFloat(formData.value)
      });
      
      if (response.success) {
        toast.success(response.message || "WHT updated successfully");
        navigate("/wht");
      } else {
        toast.error(response.message || "Failed to update WHT");
      }
    } catch (error) {
      ...
      if (error.response?.status === 422) {
        ...
        toast.error(error.response?.data?.message || "Failed to update WHT");
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="flex h-full min-h-screen bg-white-50">
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          ...
          <main className="flex-1 overflow-y-auto p-4 md:p-6">
            <div className="text-center py-8">Loading WHT data...</div>
          </main>
        </div>
      </div>
    );
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h1 className="text-2xl font-bold text-gray-900">Edit Withholding Tax</h1>
          </div>
